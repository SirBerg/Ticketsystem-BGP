...
export async function getServerSideProps(context) {
    ...
    let incident = await fetchIncident()
...
export default function Handler({username, abteilungen, mitarbeiter, prioritäten, incident, id}){
    ...
    useEffect(()=>{
        async function wrapper(){
            ...
            setInitialValue(incident.Mitarbeiter_Name)
            setMitarbeiter_State(components_mitarbeiter)
            setCachemitarbeiter(mitarbeiter)
            setCacheMitarbeiterAfterChange(mitarbeiter)

            let email = incident.Email
            if(email == null){
                email = "Nicht angegeben"
            }
            //standard states
            setAbteilungAssigned(incident.Abteilung_name)
            setMitarbeiter_assigned(incident.Mitarbeiter_Name)
            setKategorie(incident.Kategorie)
            setTickettyp(incident.Typ)
            //@ts-ignore
            setAuswirkung(parseInt(incident.Auswirkung.toString())-1)
            //@ts-ignore
            setDringlichkeit(parseInt(incident.Dringlichkeit.toString())-1)
            if(parseInt(incident.Dringlichkeit.toString())-1 < 0){
                //@ts-ignore
                setDringlichkeit(0)
            }
            if(parseInt(incident.Auswirkung.toString())-1 < 0){
                //@ts-ignore
                setAuswirkung(0)
            }
            setBeschreibung(incident.Beschreibung)
            setKurzbeschreibung(incident.Kurzbeschreibung)
            setEmail(email)
            setTel(incident.Tel)
            setusrNameForm(incident.Meldende_Person)
    ...
    function changeAuswirkungOnChange(e){
        ...
        console.log('DRINGLICHKEIT'+dringlichkeit)
    ...
    function changeDringlichkeitOnChange(e){
        ...
        console.log(prioritäten[selected_index][auswirkung])
    ...
    async function sendNewTicket(e){
        e.preventDefault()
        let abteilung_var = abteilung_assigned
        if(abteilung_assigned == undefined){
            abteilung_var = incident.Abteilung_Name
        }
        console.log(dringlichkeit, auswirkung)
        let data:requestObject = {
            "Abteilung": abteilung_var,
    ...
    return(
        <Box backgroundColor="#333">
            <Center h="10vh" backgroundColor="#333" color="white" fontFamily="sans-serif">
                <Heading size="2xl">Ticket NO. {incident.Incident_ID}</Heading>
            </Center>
                ...
                <Box backgroundColor="#454545" h="90%" w="80%" color="#f5f5f5" borderRadius="10px" minW="120vh">
                        ...
                        <Box h="30%" w="40%" float="right" m={[10]} position="relative">
                            ...
                            <FormControl fontFamily="mono">
                                ...
                                <Center>
                                    <Select id="status" bg="white" color='#333' onChange={(e)=>changeStatus(e)}>
                                        <option>Offen</option>
                                        <option>In Bearbeitung</option>
                                        <option>Wartet auf Kundenbestätigung</option>
                                        <option>Abgeschlossen</option>
                                    </Select>
                                </Center>
                            </FormControl>
                            <Center h="34vh">
                                <Button backgroundColor="#6CC644" onClick={(e)=>sendNewTicket(e)}><Link href={"/ticket?id="+id+'&is_edit=true&username='+username}>Speichern</Link></Button>
                            </Center>                             
                        ...
                        <Box h="30%" w="40%" float="left" position="relative" m={[10]}>
                            ...
                            <FormControl fontFamily="mono">
                                <Center>
                                    ...
                                    <Select id="auswirkung" bg="white" color="#333"  defaultValue={auswirkung_array[auswirkung]} onChange={(e)=>changeAuswirkungOnChange(e)}>
